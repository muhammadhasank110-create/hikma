...
- Name: tutor-voice-authenticated.spec.ts >> authenticated tutor narration >> shows tashkeel and numeral preferences only in the Arabic interface
- Location: e2e/tutor-voice-authenticated.spec.ts:119:3
...
Call log:
  - navigating to "http://127.0.0.1:3000/settings?lang=ar", waiting until "load"
...
```yaml
- generic [active]:
  - link "Skip to main content" [ref=f1e1] [cursor=pointer]:
    - /url: "#main-content"
  - link "Skip to AI tutor" [ref=f1e2] [cursor=pointer]:
    - /url: /tutor
  - link "Skip to learning settings" [ref=f1e3] [cursor=pointer]:
    - /url: /settings
```
...
> 124 |     await page.goto("/settings?lang=ar");
  ...
  214 |     const speak = page.getByRole("button", { name: "Speak a command" });
  215 |     await expect(speak).toBeEnabled();
  216 |     await speak.click();
  217 |     await expect(page.getByText("Listening…", { exact: true })).toBeVisible();
  218 |     await page.evaluate(() => (window as any).__hikmaRecognition.emit("Open mathematics"));
  219 |     await expect(page.getByText("Opening Mathematics", { exact: true })).toBeVisible();
  220 |   });
  221 | 
  222 |   test("reports an unsupported spoken feature and preserves a typed fallback", async ({ page }) => {
  223 |     await page.addInitScript(() => {
  224 |       class VoiceRecognition {
```