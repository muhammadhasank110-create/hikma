    ...
    <div className="min-h-screen flex" style={{ background: "#0d1f10" }}>
      ...
      <div className="hidden lg:flex flex-col justify-between w-[45%] relative overflow-hidden p-12"
        ...
        <motion.img src={WORDMARK_URL} alt="" aria-hidden="true"
          ...
          initial={{ opacity: 0, y: 40 }} animate={{ opacity: 0.2, y: 0 }} transition={{ duration: 1.2, ease: "easeOut" }}
        ...
        <motion.div className="relative z-10" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>
          <h2 className="text-5xl font-black text-white leading-tight tracking-tight mb-4">
            Learning that<br />
            <span style={{ color: "rgb(134,239,172)" }}>meets you</span><br />
            where you are.
          </h2>
          <p className="text-white/40 text-base leading-relaxed max-w-xs">
            Adaptive. Voice-first. Built for blind, dyslexic, and ADHD learners.
          </p>
        </motion.div>
        {/* Trust badges */}
        <motion.div className="space-y-2 relative z-10" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
          {["IGCSE Edexcel · Qatar MoEHE", "WCAG 2.2 AA · MADA Qatar", "Arabic + English · RTL support"].map(b => (
            <p key={b} className="text-xs text-white/25 flex items-center gap-2">
      ...
      <div className="flex-1 flex flex-col p-6 sm:p-12">
        {/* Back button — in flow, not absolute, so it never overlaps the logo */}
        ...
        <motion.div className="w-full max-w-md"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full flex items-center justify-center gap-2 py-4 rounded-xl font-bold text-white transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed mt-2"
          ...
          <p className="text-center text-sm text-white/30 mt-6">
            ...
            <button onClick={() => navigate("/signup")} className="text-emerald-400 hover:text-emerald-300 font-semibold transition-colors">
              Create free account
            </button>
          </p>
          <p className="text-center text-xs text-white/15 mt-4">Built to MADA Qatar & WCAG 2.2 AA · Free to use</p>
        </motion.div>
